...
commands for run 12:
python ../../sweep_analysis.py -p -sp -srd -d max_t_shift_100 -r 1 
...
parser.add_argument('-d','--data_folder_name',default='max_t_shift_100',type=str) #22
...
load_prefix = root_folder+"output_dirs/data_"+str(data_folder_name)+"/sweep_run"+str(run_num)+"_sig_amp"

file_list = glob.glob(load_prefix+"*.pkl")

if t0_num_exclude_list != 'None':
    for i in range(len(file_list)-1,-1,-1):
        temp = int(re.split('\.',re.split('_',file_list[i])[root_folder_seg_num+prename_seg_num+11])[0])
...
for i in range(len(file_list)):
    temp = int(re.split('\.',re.split('_',file_list[i])[root_folder_seg_num+prename_seg_num+6])[0])
...
load_prefix = root_folder+"output_dirs/data_"+str(data_folder_name)+"/sweep_run"+str(run_num)+"_amp"

file_list = glob.glob(load_prefix+"*.pkl")

if t0_num_exclude_list != 'None':
    for i in range(len(file_list)-1,-1,-1):
        temp = int(re.split('\.',re.split('_',file_list[i])[root_folder_seg_num+prename_seg_num+10])[0])
        if temp in t0_num_exclude_list:
            file_list.pop(i)
...
t0_num_list = []
for i in range(len(file_list)):
    temp = int(re.split('\.',re.split('_',file_list[i])[root_folder_seg_num+prename_seg_num+5])[0])
    amp_factor_num_list.append(temp)
    temp = int(re.split('\.',re.split('_',file_list[i])[root_folder_seg_num+prename_seg_num+10])[0])
    t0_num_list.append(temp)
...
fig_dir_top = 'figs'
fig_data_dir = '/figs_data_'+str(data_folder_name)
...
if save_result_data:
    hf = h5py.File(fig_dir+'/figs_run_'+str(data_folder_name)+'_results.hdf','w')
    ...
    hf.create_dataset('parameter_avg_list_'+str(data_folder_name),data=parameter_avg_list1,compression='gzip',compression_opts=9)
    hf.create_dataset('parameter_list0',data=parameter_list0,compression='gzip',compression_opts=9)
    hf.create_dataset('parameter_list_'+str(data_folder_name),data=parameter_list1,compression='gzip',compression_opts=9)
    if x2_marker_flag:
        hf.create_dataset('x2_parameter_avg_list0',data=x2_parameter_avg_list0,compression='gzip',compression_opts=9)
        hf.create_dataset('x2_parameter_avg_list_'+str(data_folder_name),data=x2_parameter_avg_list1,compression='gzip',compression_opts=9)
        hf.create_dataset('x2_parameter_list0',data=x2_parameter_list0,compression='gzip',compression_opts=9)
        hf.create_dataset('x2_parameter_list_'+str(data_folder_name),data=x2_parameter_list1,compression='gzip',compression_opts=9)
    hf.close()
